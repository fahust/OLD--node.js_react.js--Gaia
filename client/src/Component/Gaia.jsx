...
export default class Gaia extends React.Component {
    ...
    componentDidMount(){
      ...
      socket.on("party", data => {
        if((data.party.user[0].name === this.state.user.username && data.party.turn === 0) || 
        data.party.user[1].name === this.state.user.username && data.party.turn === 1){
          this.state.turn = 1
    ...
    render() {
      ...
      if(this.state.user === null && this.state.page === 'connect'){
        ...
        var buyCard = <div><BuyCard socket={socket} user={this.state.user} changePage={this.changePage} loadImg={this.loadImg}/></div>;
      }else if(this.state.user !== null && this.state.page ==='my cards'){
        var myCards = <div><MyCards socket={socket} user={this.state.user} changePage={this.changePage} loadImg={this.loadImg}/></div>;
      }else if(this.state.party !== null){
          
        var table = <div><Table socket={socket} user={this.state.user} party={this.state.party} changePage={this.changePage} loadImg={this.loadImg}/></div>
        var main = <div><Main turn={this.state.turn} socket={socket} user={this.state.user} party={this.state.party} changePage={this.changePage} loadImg={this.loadImg}/></div>
        var mainAdverse = <div><MainAdverse socket={socket} user={this.state.otherUser} cards={this.state.user.cards} party={this.state.party} changePage={this.changePage}/></div>
      }
