...
export default class CardMenu extends React.Component {
  ...
  componentDidMount() {
    ...
    setTimeout(() => {
      document.querySelectorAll(
        ...
        "rotate(" +
        (parseInt(this.state.rotation) + randomIntFromInterval(-1, 1)) +
        "deg) translate(" +
        (parseInt(this.props.x) + randomIntFromInterval(-1, 1)) +
        "px," +
        (parseInt(this.props.y) + randomIntFromInterval(-1, 1)) +
        "px) ";
    }, 5);
    setInterval(() => {
  ...
  render() {
    var card = (
      <span
        className={"menu-span-card span-card" + this.props.index}
        id={this.props.id}
        onMouseMove={this.upsizeDiv}
        onMouseOut={this.opacityDiv}
        onClick={() => this.props.changePage()}
      >
        <div className="menu-span-card-bright"></div>
        <img
          className="menu-card-img"
          src={process.env.PUBLIC_URL + "/emptycard.png"}
        />
