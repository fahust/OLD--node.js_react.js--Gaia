  ...
  constructor(props){
    ...
    render() {
      ...
      if(this.state.cards !== 'none'){
        ...
        var myCards = Object.keys(this.state.cards).map((card,index) => {
          return <span key={index} className="list-card">
              <Card socket={this.props.socket} user={this.props.user} deletable={1} allie={''} key={index} card={this.state.cards[card]} index={index} loadImg={this.props.loadImg}  y={0} x={0}/>
            </span>
        })
      }
      /*var myCards = this.state.cards.map((card,index) => {
        ...
        return <div><div className="list-buy-card">{myCards}</div>
        <br/>
          <br/>
