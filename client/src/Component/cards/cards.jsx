...
export default class Card extends React.Component {
  constructor(props) {
    ...
    if (
      this.props.card.hided === 1 &&
      this.props.card.owner !== this.props.user.username
    ) {
        ...
        setTimeout(() => {
          if (document.querySelectorAll(".span-card" + this.props.index)[0])
            document.querySelectorAll(
              ".span-card" + this.props.index
            )[0].style.transform =
              "rotateY(180deg) rotate(" + this.state.rotation + "deg)";
        }, 10);
        setTimeout(() => {
          if (document.querySelectorAll(".span-card" + this.props.index)[0])
            document.querySelectorAll(
              ...
              "rotateY(0deg) rotate(" + this.state.rotation + "deg)";
          this.rotateDiv();
        }, 1020 / 2);
      }
    }
  ...
  hoverDiv(event) {
    if (
      this.state.rotateCard === 1 /*&& this.state.turn === 1*/ &&
      this.state.rotation === 0
    ) {
      var div = document.querySelectorAll("#follower" + this.props.index);
      if (div[0]) {
        if (div[0].style.opacity <= 1) div[0].style.opacity = 1;
  ...
  upsizeDiv(event) {
    if (this.state.rotateCard == 1 && this.state.rotation === 0) {
      ...
      document.querySelectorAll(
        ...
        "scale(1.5) rotate(" + this.state.rotation + "deg)";
    }
  }
  ...
  buyCard() {
    if (
      this.props.user.cards.length < 60 &&
      this.props.user.monaie >= this.props.card.cost
    ) {
      var data = {
        page: 1,
        race: 0,
  ...
  opacityDiv() {
    ...
    if (div[0]) {
      ...
      document.querySelectorAll(
        ".span-card" + this.props.index
      )[0].style.transform = "scale(1) rotate(" + this.state.rotation + "deg)";
      clearInterval(this.state.interval);
  ...
  render() {
    var card = (
      <span
        className={
          "span-card " +
          this.state.classRotation +
          this.props.allie +
          " " +
          /*this.props.allie+*/ " span-card" +
          this.props.index
        }
        id={this.props.id}
        onMouseMove={this.upsizeDiv}
        /*onMouseMove={this.hoverDiv}*/ onMouseOut={this.opacityDiv}
      >
        <img
          className="card"
          style={{
            backgroundImage: `url(${process.env.PUBLIC_URL + "/dosCard.png"})`,
          }}
          src={
            this.state.img === 0
        ...
        />
        {this.state.rotateCard === 1 ? (
          <img
            className="card-embleme"
        ...
        {this.state.rotateCard === 1 ? (
          <div className="card-point-embleme">{this.props.card.point}</div>
        ) : (
          ""
        )}
        ...
        ></div>
        {this.props.card.invulnerable >= 1 ? (
          <img
            className="shield-invulnerability"
            src={process.env.PUBLIC_URL + "/shield.png"}
            alt=""
          />
        ) : (
          ""
        )}
        {this.state.deletable === 1 && this.props.user.cards.length > 40 ? (
          <div className="card-deletable" onClick={() => this.deleteCard()}>
            Sell
          </div>
        ) : (
          ""
        )}
        {this.state.buyable === 1 &&
        this.props.user.cards.length < 60 &&
        this.props.user.monaie >= this.props.card.cost ? (
          <div className="card-deletable" onClick={() => this.buyCard()}>
            Buy
          </div>
        ) : (
          ""
    ...
    return (
      <span
        className={
          this.props.rotation === 1 || this.state.rotateCard !== 1
            ? ""
            : "main-span-card"
        }
      >
        ...
        <span className="border-card" id={"follower" + this.props.index}>
          <div className="border-card-font">
            <div className="border-card-font-title">{this.props.card.name}</div>
            <div className="border-card-font-skillMeleedesc">
              {this.props.card.skillMelee &&
              this.props.card.skillMelee.val != ""
                ? "Melee Skill : " + this.props.card.skillMelee.val
                : ""}
            </div>
            <div className="border-card-font-skillDistdesc">
              {this.props.card.skillDist && this.props.card.skillDist.val != ""
                ? "Distant Skill : " + this.props.card.skillDist.val
                : ""}
            </div>
          </div>
          <img
          ...
          />
          <img
            className="race-icons"
            src={
              process.env.PUBLIC_URL + "/race/" + this.props.card.race + ".png"
            }
            alt=""
          />
        </span>
